// Secreto — Real Private Solana Transactions
(function () {
  'use strict';
  ...
  console.log('[Secreto] App loaded, DOM ready');
  ...
  async function connectWallet() {
    console.log('[Secreto] Connect button clicked');
  ...
  async function buildTransaction(recipient, amount) {
    ...
    if (privacyLevel !== 'standard') {
      ...
      console.log('[Secreto] Jito tip:', tipLamports, 'lamports', '(' + tipInputVal + ' SOL)');
  ...
  async function sendDirect(signedTx) {
    console.log('[Secreto] Sending via RPC proxy...');
  ...
  async function sendViaJitoTx(signedTx) {
    ...
    try {
      console.log('[Secreto] Trying Jito single-tx...', JITO_TX_URL);
      ...
      console.log('[Secreto] Jito TX response:', json);
      if (json.result) return json.result;
      if (json.error) console.warn('[Secreto] Jito TX error:', json.error);
    } catch (e) {
      console.warn('[Secreto] Jito single-tx failed, fallback to direct:', e);
    }
  ...
  async function sendViaJitoBundle(signedTx) {
    ...
    const bs58Encoded = encodeBase58(raw);
    try {
      console.log('[Secreto] Trying Jito bundle...', JITO_BUNDLE_URL);
      ...
      const json = await resp.json();
      console.log('[Secreto] Jito bundle response:', json);
      if (json.result) return json.result;
      if (json.error) console.warn('[Secreto] Jito bundle error:', json.error);
    } catch(e) {
      console.warn('[Secreto] Jito bundle failed, using single-tx:', e);
    }
    ...
    try {
      ...
      if ((privacyLevel === 'enhanced' || privacyLevel === 'maximum' || privacyLevel === 'maximum-plus') && window.ShadowPrivacy) {
        ...
        const onProgress = (hopId, title, detail) => {
          console.log('[Secreto]', hopId, title, detail);
  ...
  // ===== EVENT LISTENERS =====
  console.log('[Secreto] Registering event listeners...');

  connectBtn.addEventListener('click', () => {
    console.log('[Secreto] Connect btn click event fired');
    connectWallet();
  });
  ...
  document.querySelectorAll('.privacy-opt').forEach(el => {
    el.addEventListener('click', () => {
      ...
      console.log('[Secreto] Privacy level:', privacyLevel);
      // Update explainer
  ...
  window.addEventListener('load', async () => {
    console.log('[Secreto] Window loaded, checking for wallets...');
    ...
    for (const [key, w] of Object.entries(WALLETS)) {
      ...
      if (provider && provider.isConnected && provider.publicKey) {
        ...
        onWalletConnected();
        console.log('[Secreto] Auto-connected to', w.name);
        break;
      }
    }
  ...
  console.log('[Secreto] Init complete ✓');
})();
